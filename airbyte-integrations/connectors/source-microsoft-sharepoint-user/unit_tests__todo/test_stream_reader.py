...
from source_microsoft_sharepoint_user.stream_reader import (
    FileReadMode,
    MicrosoftSharePointUserRemoteFile,
    SourceMicrosoftSharePointUserClient,
    SourceMicrosoftSharePointUserStreamReader,
)
...
def setup_reader_class():
    reader = SourceMicrosoftSharePointUserStreamReader()  # Instantiate your class here
    config = Mock(spec=SourceMicrosoftSharePointUserSpec)
    ...
    with patch("source_microsoft_sharepoint_user.stream_reader.SourceMicrosoftSharePointUserClient") as mock_client_class:
        mock_client = mock_client_class.return_value
        mock_client.client = Mock()  # Mock the client attribute of SourceMicrosoftSharePointUserClient
        yield reader
...
def setup_client_class():
    ...
    with patch("source_microsoft_sharepoint_user.stream_reader.ConfidentialClientApplication") as mock_client_class:
        mock_msal_app_instance = Mock()
        mock_client_class.return_value = mock_msal_app_instance

        client_class = SourceMicrosoftSharePointUserClient(config)
...
@patch("source_microsoft_sharepoint_user.stream_reader.execute_query_with_retry")
@patch("source_microsoft_sharepoint_user.stream_reader.SourceMicrosoftSharePointUserStreamReader.filter_files_by_globs_and_start_date")
def test_get_matching_files(mock_filter_files, mock_execute_query, setup_reader_class, mock_drive_files, mock_shared_drive_files):
    ...
    assert isinstance(files[0], MicrosoftSharePointUserRemoteFile)
    assert files[0].uri == "file1.csv"
    assert "https://example.com/file1.csv" in files[0].download_url

    assert isinstance(files[1], MicrosoftSharePointUserRemoteFile)
...
def test_open_file(mock_smart_open, file_extension, expected_compression):
    """Test the open_file method in SourceMicrosoftSharePointUserStreamReader."""
...
def test_microsoft_sharepoint_client_initialization(requests_mock):
    """Test the initialization of SourceMicrosoftSharePointUserClient."""
    config = {
        "credentials": {
            "auth_type": "Client",
            "client_id": "client_id",
    ...
    client = SourceMicrosoftSharePointUserClient(SourceMicrosoftSharePointUserSpec(**config))

    assert client.config == SourceMicrosoftSharePointUserSpec(**config)
    assert client._msal_app is not None


def test_list_directories_and_files():
    """Test the list_directories_and_files method in SourceMicrosoftSharePointUserStreamReader."""
...
@patch("source_microsoft_sharepoint_user.stream_reader.SourceMicrosoftSharePointUserStreamReader._list_directories_and_files")
...
@patch("source_microsoft_sharepoint_user.stream_reader.execute_query_with_retry", side_effect=lambda x: x)
def test_get_shared_files_from_all_drives(
    ...
    with patch.object(
        SourceMicrosoftSharePointUserStreamReader, "_get_shared_drive_object", return_value=expected_result
        ...
        with patch.object(SourceMicrosoftSharePointUserStreamReader, "one_drive_client", new_callable=PropertyMock) as mock_one_drive_client:
...
@patch("source_microsoft_sharepoint_user.stream_reader.requests.get")
@patch("source_microsoft_sharepoint_user.stream_reader.SourceMicrosoftSharePointUserStreamReader.get_access_token")
def test_get_shared_drive_object(
    ...
    reader = SourceMicrosoftSharePointUserStreamReader()
...
def test_drives_property(auth_type, user_principal_name, has_refresh_token):
    with patch("source_microsoft_sharepoint_user.stream_reader.execute_query_with_retry") as mock_execute_query, patch(
        "source_microsoft_sharepoint_user.stream_reader.SourceMicrosoftSharePointUserStreamReader.one_drive_client"
...
def test_retrieve_files_from_accessible_drives(mocker, refresh_token, auth_type, search_scope, expected_methods_called):
    ...
    with patch.object(SourceMicrosoftSharePointUserStreamReader, "drives", return_value=[]) as mock_drives:
