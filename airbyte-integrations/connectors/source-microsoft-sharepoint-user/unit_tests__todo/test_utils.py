...
from source_microsoft_sharepoint_user.utils import execute_query_with_retry, filter_http_urls
...
def test_execute_query_with_retry(status_code, retry_after_header, expected_retries, error_message):
    ...
    with patch("source_microsoft_sharepoint_user.utils.time.sleep") as mock_sleep, patch(
        "source_microsoft_sharepoint_user.utils.datetime"
    ) as mock_datetime:
